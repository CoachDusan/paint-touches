...
| `test_cache_neighbours.py` | That shipping an update clears only **this app's** old offline copies. Every app of Dusan's shares one origin, so a service worker that clears "everything that isn't mine" leaves the neighbours unable to open without signal. Runs against a copy of the repo, bumping the version between two loads |
| `test_report.py` | The coach report: every figure and sentence `buildReport()` writes, checked against possessions built to make each number obvious; the detail tables, their grey thin-sample rows and earned green/red; the last game against the rest, including that a real change is coloured and a clip time comes from the timestamps; that it stays silent on a four-possession sample; and the screen itself, reached from Season |
