...
| `test_report.py` | The coach report: every figure and sentence `buildReport()` writes, checked against possessions built to make each number obvious; that it stays silent on a four-possession sample; and the screen itself, reached from Season |
