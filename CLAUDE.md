...
- `js/report.js` / `js/views/report.js` — the head-coach report (Season → "Open the coach report"): key numbers, wins vs losses, findings, priorities, caveats, then the offense and defense detail tables, then the last game set against everything before it (with each breakdown's and turnover's clip time, so it can be found on video). Printable as it stands. `report.js` is pure — games in, figures and sentences out — so what it claims is testable without a browser, and it calls `computeStats()`/`computeDefenseStats()` rather than re-deriving anything. **Every sentence is written from the data at build time, never stored**, and each one carries the rule it needs: a gap wide enough to matter, a sample of at least 20 possessions. One more game turned a real "3rd quarter dip" into a 0.02 difference, which is exactly why. Silence beats a claim the numbers no longer support. `tools/coach_report.py` is the same report as a PDF built on the Mac from an exported CSV; it came first and is where the wording was worked out

  **One game's report** (History → a game → "Open this game's report") is the same builder handed that game alone, with `buildReport(entries, { before })` supplying the games finished *before* it for the comparison — never the ones after, so a game's report reads the same in March as the morning after. `gameOrder()` in `report.js` is the one definition of "before". A single game is where the sentence rules get tested hardest, so each one had to be made to say what its numbers say, not what a season's usually do: no "held in 1 of 1 games"; a paint-touch gap that is only turnovers says "the gap is gone", never "real, but smaller" (Revda I, 29 Aug, is the real case); the breakdown cost and the threes comparison need 20 on each side like everything else; defensive quarters name the quarter that actually peaks instead of always saying "sharpest late"; and a tie between quarters is no finding. `tools/coach_report.py` carries the same rules, and the two should never disagree. Its pages are a fixed A4, and the game logs grow a row per game — at nine games it needed tighter rows to fit, and that buys only a few more games
